...
def register(s) -> None:
    ...
    s.has("hypr layout from keymap", hypr, "active_keymap")
    s.lacks("layout no device prefix", read(SHELL / "services" / "Layout.qml"), "slice(0, 2)")
    s.lacks("layout no ru substring", read(SHELL / "services" / "Layout.qml"), 'indexOf("ru")')
